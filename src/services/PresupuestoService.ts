...
export class PresupuestoService {
    ...
    public agregarItem(nota: NotaConceptual, nroItem: string, descripcionItem: string, nombreBienServicio: string, cantidad: number, valorUnitario: number): IResultadoValidacion {
        ...
        const item = new ItemPresupuesto(IdGenerator.generar("ITEM"), nroItem, descripcionItem, nombreBienServicio, cantidad, valorUnitario);
    ...
    public agregarItemEntidadCooperante(entidad: EntidadCooperante, detalle: string, cantidad: number, valorUnitario: number): IResultadoValidacion {
        const vCantidad = PresupuestoValidator.validarCantidad(cantidad);
        if (!vCantidad.valido) return vCantidad;
        const vValor = PresupuestoValidator.validarValorUnitario(valorUnitario);
        if (!vValor.valido) return vValor;
