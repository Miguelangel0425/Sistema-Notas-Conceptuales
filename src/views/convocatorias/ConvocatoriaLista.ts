...
export class ConvocatoriaListaView implements IView {
  ...
  public render(): HTMLElement {
    ...
    btnNueva.addEventListener("click", () =>
      Router.obtenerInstancia().navegar("#/convocatorias/nueva")
    );
    ...
    if (convocatorias.length === 0) {
      ...
      convocatorias.forEach((c) => {
        ...
        btnEliminar.addEventListener("click", () => {
          ConfirmacionModal.confirmar(
            `¿Eliminar la convocatoria "${c.nombre}"?`,
            () => {
              ...
              if (r.valido) {
                ...
                Alerta.error(r.mensaje ?? "No se pudo eliminar.");
              }
            }
          );
        });
